...
@Controller
@Scope("prototype")
public class RbacRoleEditComposer extends BasePortletComposer {
	...
	public void onOk() throws PortalException, SystemException {
		...
		if ("addChildNode".equals(opType) || "addRootNode".equals(opType)) {
			List<RbacRoleExtAttr> beanList = this.bean.getRbacRoleExtAttrExt()
					.getExtendValueList();
			rbacRole.setRbacRoleExtAttrList(beanList);
			rbacRoleManager.saveRbacRole(rbacRole);
			rbacRoleManager.updateRbacRoleToRaptornuke(rbacRole);
		} else if ("mod".equals(opType)) {
