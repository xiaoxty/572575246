...
public class StaffRoleTreeExt extends Div implements IdSpace {

	private static final long serialVersionUID = 1L;

	private final String zul = "/pages/staff_role/comp/staff_role_tree_ext.zul";
	...
	public StaffRoleTreeExt() {
		Executions.createComponents(this.zul, this, null);
		Components.wireVariables(this, this);
		Components.addForwards(this, this, '$');
		this.addForward(StaffRoleConstants.ON_CHANGE_TREE_STYLE, this, "onChangeTreeStyleResponse");
		this.addForward(StaffRoleConstants.ON_SAVE_ROLEN, this, "onSaveRole");
	}
	
	public void onSaveRole(ForwardEvent event) throws Exception {
		if (event.getOrigin().getData() != null) {
			StaffRole staffRole = (StaffRole) event.getOrigin().getData();
			this.staffRoleTree.getSelectedItem().setLabel(staffRole.getRoleName());
		}
	}
	...
	public void onAddChildNode() throws Exception {
		if (!PlatformUtil.checkPermissionDialog(getPortletInfoProvider(), ActionKeys.DATA_OPERATING))
			return;
		if (this.staffRoleTree.getSelectedItem() != null) {
			String opType = "addChildNode";
			//子节点不能新增节点
			if (staffRole == null || !StaffRoleConstants.IS_PARENT.equals(staffRole.getIsParent())) {
	...
	public void onDelNode() throws Exception {
		if (!PlatformUtil.checkPermissionDialog(getPortletInfoProvider(), ActionKeys.DATA_OPERATING))
			return;
		if (this.staffRoleTree.getSelectedItem() != null) {
			Treechildren treechildren = this.staffRoleTree.getSelectedItem().getTreechildren();
			...
			ZkUtil.showQuestion("你确定要删除该节点吗？", "提示信息", new EventListener() {
				...
				public void onEvent(Event event) throws Exception {
					...
					if (result == Messagebox.OK) {
						...
						Treechildren treechildren = (Treechildren) staffRoleTree.getSelectedItem().getParent();
						treechildren.removeChild(staffRoleTree.getSelectedItem());
						 ...
						 * 抛出删除节点成功事件
						 */
						Events.postEvent(StaffRoleConstants.ON_DEL_NODE_OK, StaffRoleTreeExt.this, null);
					}
				}
			});
		} else {
	...
	public void onSelect$staffRoleTree() throws Exception {
		staffRole = (StaffRole) ((TreeNodeImpl) this.staffRoleTree.getSelectedItem().getValue()).getEntity();
		Events.postEvent(StaffRoleConstants.ON_SELECT_TREE_ROLE, this, staffRole);
	}
	...
	public void onChangeTreeStyleResponse(ForwardEvent event) throws Exception {
		if (event.getOrigin().getData() != null) {
			...
			if (isChange != null) {
				Treeitem ti = this.staffRoleTree.getSelectedItem();
	...
	private void openAddNodeWindow(String opType) throws Exception {
		...
		Window win = (Window) Executions.createComponents("/pages/staff_role/role_edit.zul", this, map);
		win.doModal();
		win.addEventListener("onOK", new EventListener() {
			@Override
			public void onEvent(Event event) throws Exception {
				StaffRole staffRole = (StaffRole) event.getData();
				if (map.get("opType").equals("addRootNode")) {
					onAddRootNodeResponse(staffRole);
				} else if (map.get("opType").equals("addChildNode")) {
					onAddChildNodeResponse(staffRole);
				}
			}
		});
	}
	 ...
	 */
	@SuppressWarnings("rawtypes")
	private void onAddRootNodeResponse(StaffRole staffRole) throws Exception {
		if(null == staffRole){
			return;
		}
		Treechildren treechildren = this.staffRoleTree.getTreechildren();
		Treeitem treeitem = new Treeitem();
		Treerow treerow = new Treerow();
		Treecell treecell = new Treecell(staffRole.getRoleName());
		treecell.setParent(treerow);
		treerow.setParent(treeitem);
		TreeNodeImpl treeNodeImpl = new TreeNodeImpl<TreeNodeEntity>(staffRole);
		treeitem.setValue(treeNodeImpl);
		treechildren.appendChild(treeitem);
	}
	
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void onAddChildNodeResponse(StaffRole staffRole) throws Exception {
		if(null == staffRole){
			return;
		}
		Treechildren treechildren = this.staffRoleTree.getSelectedItem().getTreechildren();
		...
		if (treechildren == null) {
			 ...
			 */
			TreeNodeImpl parentTreeNodeImpl = (TreeNodeImpl) this.staffRoleTree.getSelectedItem().getValue();
			parentTreeNodeImpl.setChildren(null);
			this.staffRoleTree.getSelectedItem().setValue(parentTreeNodeImpl);

			Treechildren tchild = new Treechildren();
			Treeitem titem = new Treeitem();
			Treerow trow = new Treerow();
			Treecell tcell = new Treecell(staffRole.getLabel());
			/**
			 * 单位label样式
			 */
			...
			tchild.setParent(this.staffRoleTree.getSelectedItem());
		} else {
			// 已存在下级
			Treeitem titem = new Treeitem();
			Treerow trow = new Treerow();
	...
	public void setPagePosition()  throws Exception {
		...
		if (PlatformUtil.isAdmin()) {
			...
			if (PlatformUtil.checkHasPermission(getPortletInfoProvider(), ActionKeys.STAFF_ROLE_TREE_ADD_ROOT)) {
				canAddRoot = true;
			}
			if (PlatformUtil.checkHasPermission(getPortletInfoProvider(), ActionKeys.STAFF_ROLE_TREE_ADD_CHILD)) {
				canAddChild = true;
			}
			if (PlatformUtil.checkHasPermission(getPortletInfoProvider(), ActionKeys.STAFF_ROLE_TREE_DEL)) {
				candelChild = true;
			}
		}
