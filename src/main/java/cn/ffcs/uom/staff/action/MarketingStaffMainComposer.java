...
import cn.ffcs.uom.staff.action.bean.MarketingStaffMainBean;
...
@Controller
@Scope("prototype")
public class MarketingStaffMainComposer  extends BasePortletComposer implements IPortletInfoProvider{

	private static final long serialVersionUID = 1L;
	
	private MarketingStaffMainBean bean = new MarketingStaffMainBean();
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		bean.getStaffListboxExt().addForward(SffOrPtyCtants.ON_STAFF_MANAGE_QUERY,
				comp, "onSelectStaffResponse");
		 ...
		 */
		Events.postEvent(SffOrPtyCtants.ON_STAFF_Org_QUERY, this.bean
				.getStaffOrganizationListbox(), staff);
		/**
		 * 员工角色listbox注册
		 */
		bean.setStaffRoleListbox(this.self.getFellow("staffRoleListbox").getFellow("staffRoleMainWin"));
		/**
		 * 员工系统listbox注册
		 */
		bean.setStaffSysListbox(this.self.getFellow("staffSysListbox").getFellow("staffSysMainWin"));
	...
	public void onSaveRoleResponse(ForwardEvent event) throws Exception {
		if (event.getOrigin().getData() != null) {
			...
			if (staff != null) {
				Events.postEvent(SffOrPtyCtants.ON_STAFF_PAGE_SELECT_FOR_ROLE, this.bean.getStaffRoleListbox(), staff);
			}
		}
	}
	 ...
	 *             异常
	 */
	public void onCreate$marketingStaffMainWin(){
		try {
            bindBox();
            initPage();
        } catch (Exception e) {
	 ...
	 * 2013-5-28 Wong
	 */
    public void onSelectStaffResponse(final ForwardEvent event) throws Exception {
        ...
        try {
			...
			if (staff != null) {
				...
				if ("staffOrgTab".equals(tab)) {
					Events.postEvent(SffOrPtyCtants.ON_STAFF_Org_QUERY, this.bean.getStaffOrganizationListbox(), staff);
					...
					Events.postEvent(SffOrPtyCtants.ON_STAFF_ORG_TRAN_SELECT, bean.getStaffOrgTranListboxExt(), staff);
				} else if ("staffRoleTab".equals(tab)) {
					Events.postEvent(SffOrPtyCtants.ON_STAFF_PAGE_SELECT_FOR_ROLE, bean.getStaffRoleListbox(), staff);
				} else if ("staffSysTab".equals(tab)) {
					Events.postEvent(SffOrPtyCtants.ON_STAFF_PAGE_SELECT_FOR_SYS, bean.getStaffSysListbox(), staff);
    ...
    public void onStaffClearTabsRespons(final ForwardEvent event){
        Events.postEvent(SffOrPtyCtants.ON_CLEAN_STAFF_ORG, bean.getStaffOrganizationListbox(), null);
        Events.postEvent(SffOrPtyCtants.ON_CLEAN_STAFF_POSITI, bean.getStaffPositionListboxExt(), null);
        Events.postEvent(SffOrPtyCtants.ON_CLEAN_STAFFORG_TRAN, bean.getStaffOrgTranListboxExt(), null);
    }
	...
	private void initPage() throws Exception {
		this.bean.getStaffListboxExt().setPagePosition("marketingStaffPage");
		this.bean.getStaffOrganizationListbox().setPagePosition("marketingStaffPage");
		this.bean.getStaffPositionListboxExt().setPagePosition("marketingStaffPage");
		this.bean.getStaffOrgTranListboxExt().setPagePosition("marketingStaffPage");
		this.bean.getPartyContactInfoListboxExt().setPagePosition("marketingStaffPage");
		this.bean.getPartyCertificationListboxExt().setPagePosition("marketingStaffPage");
		this.bean.getPartyRoleListboxExt().setPagePosition("marketingStaffPage");
	}
}
