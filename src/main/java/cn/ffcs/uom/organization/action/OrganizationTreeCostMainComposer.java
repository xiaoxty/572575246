...
import cn.ffcs.uom.organization.action.bean.OrganizationCostTreeMainBean;
...
@Controller
@Scope("prototype")
public class OrganizationTreeCostMainComposer extends BasePortletComposer
	 ...
	 * 页面bean
	 */
	private OrganizationCostTreeMainBean bean = new OrganizationCostTreeMainBean();
	/**
	 * 选中的组织
	 */
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		this.bean.getCostTreeExt().setPortletInfoProvider(this);

		/**
		 * 选中组织树上的组织
		 */
		this.bean.getCostTreeExt().addForward(
	...
	private void initPage() throws Exception {
		this.bean.getCostTreeExt().setPagePosition("costTreePage");
	...
	public void initLeftTab() throws Exception {
		...
		if (!checkCostTabResult) {
			...
			this.bean.getCostTab().setVisible(false);
			ZkUtil.showExclamation("您没有任何菜单权限,请配置", "警告");
		} else {
			this.bean.getCostTab().setSelected(true);
		}
		...
		if (organization != null) {
			...
			if ("orgTranTab".equals(this.bean.getSelectTab().getId())) {
				this.bean.getOrganizationTranListboxExt()
						.setVariableOrgTreeTabName("costTab");
	...
	public void callLeftTab() throws Exception {
		...
		if ("costTab".equals(bean.getLeftSelectTab().getId())) {
			organizationRelation = this.bean.getCostTreeExt()
				...
				if ("costTab".equals(bean.getLeftSelectTab().getId())) {
					...
					if (org.isCompany()) {
						Events.postEvent(
								...
								this.bean.getCostTreeExt(), true);
					} else {
						Events.postEvent(
								OrganizationConstant.ON_CHANGE_TREE_STYLE,
								this.bean.getCostTreeExt(), false);
					}
					/**
					 * 组织信息保存可能对组织名称进行了修改
					 */
