...
import cn.ffcs.uom.organization.action.bean.OrganizationMarketingTreeMainBean;
...
@Controller
@Scope("prototype")
public class OrganizationTreeMarketingMainComposer extends BasePortletComposer
	 ...
	 * 页面bean
	 */
	private OrganizationMarketingTreeMainBean bean = new OrganizationMarketingTreeMainBean();
	/**
	 * 选中的组织
	 */
	 ...
	 *             异常
	 */
	public void onCreate$organizationMarketingTreeMainWindow() throws Exception {
		initLeftTab();
		initPage();
	...
	private void initPage() throws Exception {
		this.bean.getMarketingTreeExt().setPagePosition("marketingTreePage");
	...
	private void initLeftTabControlRightPage() throws Exception {
		String selectedTabId = "marketingTab";
		// 控制组织层级权限
		this.bean.getMarketingTreeExt().setPagePosition("marketingTreePage");
	...
	public void initLeftTab() throws Exception {
		...
		if (!checkMarketingTabResult) {
			...
			this.bean.getMarketingTab().setVisible(false);
			ZkUtil.showExclamation("您没有任何菜单权限,请配置", "警告");
		} else {
			this.bean.getMarketingTab().setSelected(true);
		}
		...
		if (returParty != null) {
			...
			if ("partyContactInfoTab".equals(tab)) {
				...
				this.bean.getPartyContactInfoListboxExt()
						.setVariableOrgTreeTabName("marketingTab");
	...
	public void callLeftTab() throws Exception {
		...
		if ("marketingTab".equals(bean.getLeftSelectTab().getId())) {
				...
				if ("marketingTab".equals(bean.getLeftSelectTab().getId())) {
					...
					if (org.isCompany()) {
						Events.postEvent(
								...
								this.bean.getMarketingTreeExt(), true);
					} else {
						Events.postEvent(
								OrganizationConstant.ON_CHANGE_TREE_STYLE,
								this.bean.getMarketingTreeExt(), false);
					}
					/**
					 * 组织信息保存可能对组织名称进行了修改
					 */
					Events.postEvent(
							...
							this.bean.getMarketingTreeExt(), org);
				}
			}
		}
	}
