...
import cn.ffcs.uom.organization.action.bean.OrganizationMainBean;
...
@Controller
@Scope("prototype")
public class OrganizationMainComposer extends BasePortletComposer implements IPortletInfoProvider{
	/**
	 * 
	 */
	 ...
	 * bean.
	 */
	private OrganizationMainBean bean = new OrganizationMainBean();
	/**
	 * 选中的组织
	 */
	...
	public void onOrganiztionQueryResponse(ForwardEvent event) throws Exception {
		 ...
		 * 清空tab数据
		 */
		Events.postEvent(OrganizationConstant.ON_ORGANIZATION_QUERY, bean
				.getOrganizationRelationListbox(), null);
		Events.postEvent(OrganizationConstant.ON_ORGANIZATION_QUERY, bean
				.getStaffOrganizationListbox(), null);
		Events.postEvent(OrganizationConstant.ON_ORGANIZATION_QUERY, bean
				.getOrganizationPositionExt(), null);
	}
	...
	public void callTab() throws Exception {
		...
		if (organization != null && organization.getOrgId() != null) {
			...
			if("uomGroupOrgTranTab".equals(this.bean.getSelectTab().getId()))
			{
                ...
                //传递组织id过去，吧所有域外
                this.bean.getUomGroupOrgTranListboxExt().setVariableOrgTreeTabName("organization");
	...
	public void onDelOrganizationResponse(ForwardEvent event) throws Exception {
		 ...
		 * 组织关系要清空
		 */
		Events.postEvent(OrganizationConstant.ON_DEL_ORGANIZAITON, bean
				.getOrganizationRelationListbox(), null);
	}
	...
	private void initPage() throws Exception {
		this.bean.getOrganizationListbox().setPagePosition("organizationPage");
		this.bean.getOrganizationRelationListbox().setPagePosition("organizationPage");
		this.bean.getStaffOrganizationListbox().setPagePosition("organizationPage");
		this.bean.getOrganizationPositionExt().setPagePosition("organizationPage");
		this.bean.getUomGroupOrgTranListboxExt().setPagePosition("organizationPage");
	}
}
