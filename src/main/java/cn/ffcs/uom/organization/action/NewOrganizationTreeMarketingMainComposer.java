...
import cn.ffcs.uom.organization.action.bean.NewOrganizationMarketingTreeMainBean;
...
@Controller
@Scope("prototype")
public class NewOrganizationTreeMarketingMainComposer extends
	 ...
	 * 页面bean
	 */
	private NewOrganizationMarketingTreeMainBean bean = new NewOrganizationMarketingTreeMainBean();
	/**
	 * 选中的组织
	 */
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		this.bean.getNewMarketingTreeExt().setPortletInfoProvider(this);
		this.bean.getNewMarketingTreeExt().setVariablePagePosition(
				"newMarketingTreePage");

		/**
		 * 选中组织树上的组织
		 */
		this.bean
				.getNewMarketingTreeExt()
				.addForward(
		 ...
		 * 删除节点成功事件
		 */
		this.bean.getNewMarketingTreeExt().addForward(
	 ...
	 *             异常
	 */
	public void onCreate$newOrganizationMarketingTreeMainWindow()
			throws Exception {
		initLeftTab();
		initPage();
	...
	private void initPage() throws Exception {
		this.bean.getNewMarketingTreeExt().setPagePosition(
				"newMarketingTreePage");
	...
	private void initLeftTabControlRightPage() throws Exception {
		String selectedTabId = "newMarketingTab";
		// 控制组织层级权限
		this.bean.getNewMarketingTreeExt().setPagePosition(
				"newMarketingTreePage");
	...
	public void initLeftTab() throws Exception {
		...
		if (!checkMarketingTabResult) {
			...
			this.bean.getNewMarketingTab().setVisible(false);
			ZkUtil.showExclamation("您没有任何菜单权限,请配置", "警告");
		} else {
			this.bean.getNewMarketingTab().setSelected(true);
		}
		...
		if (returParty != null) {
			...
			if ("partyContactInfoTab".equals(tab)) {
				...
				this.bean.getPartyContactInfoListboxExt()
						.setVariableOrgTreeTabName("newMarketingTab");
	...
	public void callLeftTab() throws Exception {
		...
		if ("newMarketingTab".equals(bean.getLeftSelectTab().getId())) {
			organizationRelation = this.bean.getNewMarketingTreeExt()
				...
				if ("newMarketingTab".equals(bean.getLeftSelectTab().getId())) {
					...
					if (org.isCompany()) {
						Events.postEvent(
								...
								this.bean.getNewMarketingTreeExt(), true);
					} else {
						Events.postEvent(
								OrganizationConstant.ON_CHANGE_TREE_STYLE,
								this.bean.getNewMarketingTreeExt(), false);
					}
					/**
					 * 组织信息保存可能对组织名称进行了修改
					 */
					Events.postEvent(
							...
							this.bean.getNewMarketingTreeExt(), org);
				}
			}
		}
	}
