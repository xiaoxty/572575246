...
import cn.ffcs.uom.organization.action.bean.OrganizationTreeMainBean;
...
@Controller
@Scope("prototype")
public class OrganizationTreeMainComposer extends BasePortletComposer
	 ...
	 * 页面bean
	 */
	private OrganizationTreeMainBean bean = new OrganizationTreeMainBean();
	/**
	 * 选中的组织
	 */
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		this.bean.getPoliticalTreeExt().setPortletInfoProvider(this);
		this.bean.getAgentTreeExt().setPortletInfoProvider(this);
		this.bean.getIbeTreeExt().setPortletInfoProvider(this);
		...
		this.bean.getSupplierTreeExt().setPortletInfoProvider(this);
		this.bean.getOssTreeExt().setPortletInfoProvider(this);
		...
		this.bean.getIbeTreeExt().addForward(
			OrganizationConstant.ON_SELECT_IBE_ORGANIZATION_TREE_REQUEST, this.self,
			"onSelectOrganizationRelationTreeResponse");
		this.bean.getPoliticalTreeExt().addForward(
			OrganizationConstant.ON_SELECT_POLITICAL_ORGANIZATION_TREE_REQUEST, this.self,
			"onSelectOrganizationRelationTreeResponse");
		this.bean.getSupplierTreeExt().addForward(
			OrganizationConstant.ON_SELECT_SUPPLIER_ORGANIZATION_TREE_REQUEST, this.self,
			"onSelectOrganizationRelationTreeResponse");
		this.bean.getOssTreeExt().addForward(
			OrganizationConstant.ON_SELECT_ORGANIZATION_TREE_REQUEST, this.self,
			"onSelectOrganizationRelationTreeResponse");
		/**
		 * 删除节点成功事件
		 */
		this.bean.getAgentTreeExt().addForward(OrganizationConstant.ON_DEL_NODE_OK, this.self,
			"onDelNodeResponse");
		/**
		 * 删除节点成功事件
		 */
		this.bean.getIbeTreeExt().addForward(OrganizationConstant.ON_DEL_NODE_OK, this.self,
			"onDelNodeResponse");
		/**
		 * 删除节点成功事件
		 */
		this.bean.getPoliticalTreeExt().addForward(OrganizationConstant.ON_DEL_NODE_OK, this.self,
			"onDelNodeResponse");
		/**
		 * 删除节点成功事件
		 */
		this.bean.getSupplierTreeExt().addForward(OrganizationConstant.ON_DEL_NODE_OK, this.self,
			"onDelNodeResponse");
		/**
		 * 选择代理树
		 */
		this.bean.getAgentTreeExt().addForward(OrganizationConstant.ON_SELECT_TREE_TYPE, this.self,
			"onSelectTreeTypeResponse");
		/**
		 * 选择内部经营实体树
		 */
		this.bean.getIbeTreeExt().addForward(OrganizationConstant.ON_SELECT_TREE_TYPE, this.self,
			"onSelectTreeTypeResponse");
		/**
		 * 选择推导树
		 */
		this.bean.getPoliticalTreeExt().addForward(OrganizationConstant.ON_SELECT_TREE_TYPE,
	 ...
	 *             异常
	 */
	public void onCreate$organizationTreeMainWindow() throws Exception {
		initLeftTab();
		initPage();
	...
	private void initPage() throws Exception {
		this.bean.getPoliticalTreeExt().setPagePosition("politicalTreePage");
		this.bean.getAgentTreeExt().setPagePosition("agentTreePage");
		this.bean.getIbeTreeExt().setPagePosition("ibeTreePage");
		this.bean.getSupplierTreeExt().setPagePosition("supplierTreePage");
		this.bean.getOssTreeExt().setPagePosition("ossTreePage");
		/**
		 * 未区分组织树各TAB页的权限分离
		 */
		...
		this.bean.getOrganizationTranListboxExt().setPagePosition("orgTreePage");
		...
		optionAttrValueList.add(OrganizationTranConstant.DEPARTMENT_NETWORK_MANY_TO_ONE);
		
		this.bean.getOrganizationTranListboxExt().getBean().getTranRelaTypeDiv().setVisible(false);
		this.bean.getOrganizationTranListboxExt().getBean().getTelcomRegionDiv().setVisible(false);
		this.bean.getUomGroupOrgTranListboxExt().getBean().getTelcomRegionDiv().setVisible(false);
	...
	public void initLeftTab() throws Exception {
		...
		boolean checkPoliticalTabResult = PlatformUtil.checkHasPermission(this,
			ActionKeys.POLITICAL_TAB);
		boolean checkAgentTabResult = PlatformUtil.checkHasPermission(this, ActionKeys.AGENT_TAB);
		boolean checkIbeTabResult = PlatformUtil.checkHasPermission(this, ActionKeys.IBE_TAB);
		boolean checkSupplierTabResult = PlatformUtil.checkHasPermission(this,
			ActionKeys.SUPPLIER_TAB);
		boolean checkOssTabResult = PlatformUtil.checkHasPermission(this, ActionKeys.OSS_TAB);
		
		if (!checkPoliticalTabResult && !checkAgentTabResult && !checkIbeTabResult
			&& !checkSupplierTabResult && !checkOssTabResult) {
			this.bean.getTempTab().setVisible(true);
			this.bean.getTempTab().setSelected(true);
			ZkUtil.showExclamation("您没有任何菜单权限,请配置", "警告");
		} else {
			if (!checkPoliticalTabResult) {
			...
			if (!checkIbeTabResult) {
				this.bean.getIbeTab().setVisible(false);
			}
			if (!checkSupplierTabResult) {
				this.bean.getSupplierTab().setVisible(false);
			}
			if (!checkOssTabResult) {
				this.bean.getOssTab().setVisible(false);
			}
			
			if (checkPoliticalTabResult) {
				this.bean.getPoliticalTab().setSelected(true);
				this.bean.setLeftSelectTab(bean.getPoliticalTab());
			} else if (checkAgentTabResult) {
				this.bean.getAgentTab().setSelected(true);
				this.bean.setLeftSelectTab(bean.getAgentTab());// 当只有一个代理商TAB页面时，解决用工性质无法选择代理商员工的BUG
			} else if (checkIbeTabResult) {
				this.bean.getIbeTab().setSelected(true);
				this.bean.setLeftSelectTab(bean.getIbeTab());
			} else if (checkSupplierTabResult) {
				this.bean.getSupplierTab().setSelected(true);
				this.bean.setLeftSelectTab(bean.getSupplierTab());
			} else if (checkOssTabResult) {
				this.bean.getOssTab().setSelected(true);
				this.bean.setLeftSelectTab(bean.getOssTab());
			}
		}
	}
	...
	public void callTab() throws Exception {
		...
		if (organization != null) {
			if ("orgAttrTab".equals(this.bean.getSelectTab().getId())) {
				Events.postEvent(OrganizationConstant.ON_SELECT_TREE_ORGANIZATION,
			...
			if ("orgRelaTab".equals(this.bean.getSelectTab().getId())) {
				...
				organizationRelation.setIsAgent("agentTab".equals(bean.getLeftSelectTab().getId()));
				organizationRelation.setIsIbe("ibeTab".equals(bean.getLeftSelectTab().getId()));
			...
			if ("orgTranTab".equals(this.bean.getSelectTab().getId())) {
				...
				if ("politicalTab".equals(bean.getLeftSelectTab().getId())) {
					...
					this.bean.getOrganizationTranListboxExt().setVariableOrgTreeTabName(
						"politicalTab");
					
				} else {
					
					this.bean.getOrganizationTranListboxExt().setVariableOrgTreeTabName("otherTab");
				}
				
				Events.postEvent(OrganizationTranConstant.ON_ORGANIZATION_TRAN_QUERY,
					this.bean.getOrganizationTranListboxExt(), organizationTran);
			...
			if ("uomGroupOrgTranTab".equals(this.bean.getSelectTab().getId())) {
				...
				if ("agentTab".equals(bean.getLeftSelectTab().getId())) {
					...
					this.bean.getUomGroupOrgTranListboxExt().setVariableOrgTreeTabName("agentTab");
					
				} else if ("ibeTab".equals(bean.getLeftSelectTab().getId())) {
					
					this.bean.getUomGroupOrgTranListboxExt().setVariableOrgTreeTabName("ibeTab");
					
				} else {
					
					this.bean.getUomGroupOrgTranListboxExt().setVariableOrgTreeTabName("otherTab");
				}
				
				Events.postEvent(OrganizationTranConstant.ON_UOM_GROUP_ORG_TRAN_QUERY,
					this.bean.getUomGroupOrgTranListboxExt(), uomGroupOrgTran);
			}
			...
			if ("orgPositionTab".equals(this.bean.getSelectTab().getId())) {
				Events.postEvent(OrganizationConstant.ON_ORGANIZATION_POSITION_QUERY,
					this.bean.getOrganizationPositionListbox(), organization);
			}
            ...
            if ("staffOrgTranTab".equals(this.bean.getSelectTab().getId())) {
                // staffOrgTranListboxExt
                Events.postEvent(OrganizationConstant.ON_SELECT_ORG_STAFF_ORGANIZATION_TRAN_QUERY,
                    this.bean.getStaffOrgTranListboxExt(), organization);
            }
		} else {
			/**
	...
	public void callLeftTab() throws Exception {
		...
		if ("politicalTab".equals(bean.getLeftSelectTab().getId())) {// 内部管理树界面
			organizationRelation = this.bean.getPoliticalTreeExt()
				.getSelectOrganizationOrganization();
			this.setTreeTypeToRightTab(bean.getPoliticalTreeExt().isDuceTree());
			this.bean.getOrganizationPartyAttrExt().setIsAgentTab(false);
			this.bean.getStaffOrganizationListbox().setIsPoliticalTab(true);
			...
			this.bean.getOrganizationRelationListbox().setIsAgentTab(false);
			this.bean.getOrganizationRelationListbox().setIsIbeTab(false);
			this.bean.getUomGroupOrgTranListboxExt().setIsAgentTab(false);
		...
		if ("agentTab".equals(bean.getLeftSelectTab().getId())) {
			organizationRelation = this.bean.getAgentTreeExt().getSelectOrganizationOrganization();
			...
			this.bean.getUomGroupOrgTranListboxExt().setIsIbeTab(false);
			this.bean.getOrganizationInfoEditExt().restoreOrganizationInfoExtStyle("class",
				"z-info-required", "* 地址");
			// 是否显示新增删除按钮
			/*
			 * Events.postEvent(OrganizationConstant.ON_IS_VISIBLE,
		...
		if ("ibeTab".equals(bean.getLeftSelectTab().getId())) {
			organizationRelation = this.bean.getIbeTreeExt().getSelectOrganizationOrganization();
			this.setTreeTypeToRightTab(false);
			this.bean.getOrganizationPartyAttrExt().setIsIbeTab(true);
			this.bean.getStaffOrganizationListbox().setIsPoliticalTab(false);
			this.bean.getStaffOrganizationListbox().setIsAgentTab(false);
			this.bean.getStaffOrganizationListbox().setIsIbeTab(true);
			this.bean.getOrganizationRelationListbox().setIsAgentTab(false);
			this.bean.getOrganizationRelationListbox().setIsIbeTab(true);
			this.bean.getUomGroupOrgTranListboxExt().setIsAgentTab(false);
			this.bean.getUomGroupOrgTranListboxExt().setIsIbeTab(true);
			this.bean.getOrganizationInfoEditExt().setIsAgentTab(false);
			this.bean.getOrganizationInfoEditExt().setIsIbeTab(true);
			this.bean.getOrganizationInfoEditExt().restoreOrganizationInfoExtStyle("class",
				"z-info-required", "* 地址");
			// 是否显示新增删除按钮
			/*
		...
		if ("supplierTab".equals(bean.getLeftSelectTab().getId())) {
			organizationRelation = this.bean.getAgentTreeExt().getSelectOrganizationOrganization();
			this.setTreeTypeToRightTab(false);
			this.bean.getOrganizationPartyAttrExt().setIsAgentTab(false);
			this.bean.getStaffOrganizationListbox().setIsPoliticalTab(false);
		...
		if ("ossTab".equals(bean.getLeftSelectTab().getId())) {
			organizationRelation = this.bean.getOssTreeExt().getSelectOrganizationOrganization();
			this.setTreeTypeToRightTab(false);
			this.bean.getOrganizationPartyAttrExt().setIsAgentTab(false);
			this.bean.getStaffOrganizationListbox().setIsPoliticalTab(false);
			this.bean.getStaffOrganizationListbox().setIsAgentTab(false);
		...
		if (event.getOrigin().getData() != null) {
			...
			if (org != null) {
				...
				if ("politicalTab".equals(bean.getLeftSelectTab().getId())) {
					 ...
					 */
					if (org.isCompany()) {
						Events.postEvent(OrganizationConstant.ON_CHANGE_TREE_STYLE,
