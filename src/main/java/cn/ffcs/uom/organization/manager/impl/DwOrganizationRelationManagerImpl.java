...
@Service("dwOrganizationRelationManager")
@Scope("prototype")
public class DwOrganizationRelationManagerImpl extends BaseDaoImpl implements
	...
	public PageInfo queryPageInfoByOrganizationRelation(
		...
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM v_mdsion_organization_relation WHERE STATUS_CD= ?");
	...
	public PageInfo queryPageInfoByOrganizationRelationNoStatusCd(
			...
			int pageSize) {
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM v_mdsion_organization_relation WHERE 1=1");
		...
		if (organization != null && organizationRelation != null) {
			...
			if (!StrUtil.isNullOrEmpty(organizationRelation.getRelaCd())) {
				sql.append(" AND RELA_CD=?");
				params.add(StringEscapeUtils.escapeSql(organizationRelation.getRelaCd().trim()));
			}
	...
	public List<OrganizationRelation> queryParentTreeOrganizationRelationList(
		...
		List params = new ArrayList();
		String sql = "SELECT T.* FROM (SELECT * FROM v_mdsion_organization_relation WHERE STATUS_CD = ? AND RELA_CD = ?) T START WITH ORG_ID = ? CONNECT BY PRIOR RELA_ORG_ID = ORG_ID";
		params.add(BaseUnitConstants.ENTT_STATE_ACTIVE);
		params.add(relaCd);
		params.add(orgId);
	...
	public boolean checkOrgRelaIsExist(Long orgId, String relaCd) {
		List params = new ArrayList();
		String sql = "select * from v_mdsion_organization_relation where org_id = ? and status_cd = ? and rela_cd = ?";
		params.add(orgId);
		params.add(BaseUnitConstants.ENTT_STATE_ACTIVE);
		params.add(relaCd);
	...
	public OrganizationRelation getOrgRelByTreeId(Long treeId) {
		List params = new ArrayList();
		StringBuffer sb = new StringBuffer(
				"SELECT * FROM v_mdsion_organization_relation WHERE STATUS_CD=? AND ORG_REL_ID=(SELECT ORG_REL_ID FROM ORG_REL_ROOT_CONFIG WHERE STATUS_CD=? AND ORG_TREE_ID=?)");
		params.add(BaseUnitConstants.ENTT_STATE_ACTIVE);
		params.add(BaseUnitConstants.ENTT_STATE_ACTIVE);
		params.add(treeId);
