...
public class IbeTreeExt extends Div implements IdSpace {

	/**
	 * 
	 */
	private static final long serialVersionUID = -557504677908050664L;
	/**
	 * zul文件路径。
	 */
	private final String zul = "/pages/organization/ibe_tree_ext.zul";
	/**
	 * 组织树
	 */
	private OrganizationRelationTree ibeOrganizationRelationTree;
	/**
	 * 组织树跟节点
	 */
	...
	public void onCreate() {
		this.ibeOrganizationRelationTree.setVisible(false);
	}
	...
	public void onChanging$organizationRoot() throws Exception {
		...
		if (organization != null) {
			ibeOrganizationRelationTree.setRootId(organization.getOrgId());
			ibeOrganizationRelationTree.bindTree();
			ibeOrganizationRelationTree.setVisible(true);
		} else {
			ibeOrganizationRelationTree.setVisible(false);
		}
	 ...
	 * @throws Exception
	 */
	public void onSelect$ibeOrganizationRelationTree() throws Exception {
		organizationRelation = (OrganizationRelation) ((TreeNodeImpl) this.ibeOrganizationRelationTree
		...
		Events.postEvent(
				OrganizationConstant.ON_SELECT_IBE_ORGANIZATION_TREE_REQUEST,
	...
	private void onAddRootNodeResponse(OrganizationRelation organizationRelation)
		...
		if (organizationRelation != null
				&& organizationRelation.getOrgId() != null) {
			this.ibeOrganizationRelationTree.setRootId(organizationRelation
					.getOrgId());
			this.ibeOrganizationRelationTree.bindTree();
			this.ibeOrganizationRelationTree.setVisible(true);
		}
	}
	...
	public void onAddChildNode() throws Exception {
		if (!PlatformUtil.checkPermissionDialog(getPortletInfoProvider(),
				ActionKeys.DATA_OPERATING))
			return;
		if (this.ibeOrganizationRelationTree.getSelectedItem() != null) {
			String opType = "addIbeChildNode";
			this.openAddNodeWindow(opType);
		} else {
	...
	private void onAddChildNodeResponse(
		...
		Treechildren treechildren = this.ibeOrganizationRelationTree
		...
		if (treechildren == null) {
			 ...
			 */
			TreeNodeImpl parentTreeNodeImpl = (TreeNodeImpl) this.ibeOrganizationRelationTree
					.getSelectedItem().getValue();
			parentTreeNodeImpl.setChildren(null);
			this.ibeOrganizationRelationTree.getSelectedItem().setValue(
					parentTreeNodeImpl);
			...
			tchild.setParent(this.ibeOrganizationRelationTree.getSelectedItem());
		} else {
			// 已存在下级
			Treeitem titem = new Treeitem();
			Treerow trow = new Treerow();
	...
	private void openAddNodeWindow(String opType) throws Exception {
		...
		win.addEventListener("onOK", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				if (map.get("opType").equals("addIbeRootNode")) {
					onAddRootNodeResponse(organizationRelation);
				} else if (map.get("opType").equals("addIbeChildNode")) {
			...
			window.addEventListener(Events.ON_OK, new EventListener() {
				...
				public void onEvent(Event event) throws Exception {
					if (event.getData() != null) {
						...
						Treechildren treechildren = (Treechildren) ibeOrganizationRelationTree
								.getSelectedItem().getParent();
						treechildren.removeChild(ibeOrganizationRelationTree
								.getSelectedItem());
	...
	public void setPagePosition(String page) throws Exception {
		...
		if (PlatformUtil.isAdmin()) {
			...
			canAddChild = true;
			candelChild = true;
		} else if ("ibeTreePage".equals(page)) {
			if (PlatformUtil.checkHasPermission(getPortletInfoProvider(),
					ActionKeys.IBE_TREE_ORG_ADD_ROOT)) {
				canAddRoot = true;
			}
			if (PlatformUtil.checkHasPermission(getPortletInfoProvider(),
					ActionKeys.IBE_TREE_ORG_ADD_CHILD)) {
				canAddChild = true;
			}
			if (PlatformUtil.checkHasPermission(getPortletInfoProvider(),
					ActionKeys.IBE_TREE_ORG_DEL)) {
				candelChild = true;
			}
		}
