...
public class MarketingTreeExt extends Div implements IdSpace {
	 ...
	 * zul文件路径。
	 */
	private final String zul = "/pages/organization/marketing_tree_ext.zul";
	/**
	 * 组织树
	 */
	...
	public MarketingTreeExt() {
		 ...
		 * 组织名称修改要改组织树节点名称
		 */
		MarketingTreeExt.this.addForward(
				OrganizationConstant.ON_SAVE_ORGANIZATION_INFO,
				MarketingTreeExt.this, "onSaveOrganiazationInfoResponse");
	...
	public void onAddRootNode() throws Exception {
		...
		String opType = "addMarketingRootNode";
	...
	public void onAddChildNode() throws Exception {
		...
		if (this.organizationRelationTree.getSelectedItem() != null) {
			String opType = "addMarketingChildNode";
			this.openAddNodeWindow(opType);
		} else {
	...
	private void openAddNodeWindow(String opType) throws Exception {
		...
		win.addEventListener("onOK", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				if (map.get("opType").equals("addMarketingRootNode")) {
					onAddRootNodeResponse(organizationRelation);
				} else if (map.get("opType").equals("addMarketingChildNode")) {
	...
	public void onSelect$organizationRelationTree() throws Exception {
		...
		// 组织树层级控制
		// this.setPagePosition("marketingTreePage");
