...
public class CostTreeExt extends Div implements IdSpace {
	 ...
	 * zul文件路径。
	 */
	private final String zul = "/pages/organization/cost_tree_ext.zul";
	/**
	 * 组织树
	 */
	...
	public CostTreeExt() {
		 ...
		 * 组织名称修改要改组织树节点名称
		 */
		CostTreeExt.this.addForward(
				OrganizationConstant.ON_SAVE_ORGANIZATION_INFO,
				CostTreeExt.this, "onSaveOrganiazationInfoResponse");
	...
	public void onAddChildNode() throws Exception {
		...
		if (this.organizationRelationTree.getSelectedItem() != null) {
			String opType = "addCostChildNode";
			this.openAddNodeWindow(opType);
		} else {
	...
	private void openAddNodeWindow(String opType) throws Exception {
		...
		win.addEventListener("onOK", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				if (map.get("opType").equals("addCostRootNode")) {
					onAddRootNodeResponse(organizationRelation);
				} else if (map.get("opType").equals("addCostChildNode")) {
	...
	public void setPagePosition(String page) throws Exception {
		...
		if (PlatformUtil.isAdmin()) {
			...
			canAddChild = true;
			candelChild = true;
		} else if ("costTreePage".equals(page)) {
