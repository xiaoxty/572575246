...
public class JtMssCostRelationTree extends Tree implements IdSpace {
	/**
	 * 树类型
	 */
	@Getter
	...
	public void bindTree(Long orgTreeId) throws Exception {
		JtMssCostRelation orgRel = new JtMssCostRelation();
		...
		final TreeNodeImpl<JtMssCostRelation> treeNode = new TreeNodeImpl<JtMssCostRelation>(
				orgRel);
	...
	public void setLableStyle() {
		...
		for (Treeitem ti : tis) {
			JtMssCostRelation jtMssCostRelation = (JtMssCostRelation) ((TreeNodeImpl) ti
					.getValue()).getEntity();
