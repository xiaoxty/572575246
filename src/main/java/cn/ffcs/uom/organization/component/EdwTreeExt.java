...
public class EdwTreeExt extends Div implements IdSpace {
	 ...
	 * zul文件路径。
	 */
	private final String zul = "/pages/organization/edw_tree_ext.zul";
	/**
	 * 组织树
	 */
	...
	public EdwTreeExt() {
		 ...
		 * 组织名称修改要改组织树节点名称
		 */
		EdwTreeExt.this.addForward(
				OrganizationConstant.ON_SAVE_ORGANIZATION_INFO,
				EdwTreeExt.this, "onSaveOrganiazationInfoResponse");
	...
	private void onAddChildNodeResponse(
		...
		if (treechildren == null) {
			...
			tchild
					.setParent(this.organizationRelationTree
							.getSelectedItem());
		} else {
			// 已存在下级
			Treeitem titem = new Treeitem();
	...
	private void openAddNodeWindow(String opType) throws Exception {
		...
		win.addEventListener("onOK", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				if (map.get("opType").equals("addEdwRootNode")) {
					onAddRootNodeResponse(organizationRelation);
				} else if (map.get("opType").equals("addEdwChildNode")) {
	...
	public void onSelect$organizationRelationTree() throws Exception {
		...
		Events
				.postEvent(
						OrganizationConstant.ON_SELECT_EDW_ORGANIZATION_TREE_REQUEST,
	...
	private void changeOrgSeq() throws Exception {
		Treeitem selectItem = this.organizationRelationTree
				.getSelectedItem();
	...
	public void setPagePosition(String page) throws Exception {
		...
		if (PlatformUtil.isAdmin()) {
			...
			canAddChild = true;
			candelChild = true;
		} else if ("edwTreePage".equals(page)) {
			if (PlatformUtil.checkHasPermission(getPortletInfoProvider(),
					ActionKeys.EDW_TREE_ORG_ADD_ROOT)) {
				canAddRoot = true;
			}
			if (PlatformUtil.checkHasPermission(getPortletInfoProvider(),
					ActionKeys.EDW_TREE_ORG_ADD_CHILD)) {
				canAddChild = true;
			}
			if (PlatformUtil.checkHasPermission(getPortletInfoProvider(),
					ActionKeys.EDW_TREE_ORG_DEL)) {
				candelChild = true;
			}
		}
