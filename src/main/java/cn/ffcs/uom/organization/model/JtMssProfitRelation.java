 ...
 * 
 **/
public class JtMssProfitRelation implements TreeNodeEntity,
		Serializable {
	 ...
	 */
	public static JtMssProfitRelation newInstance() {
		return new JtMssProfitRelation();
	}
		...
		if (this.subSetName != null) {
			if (treeCalcVo == null) {
				...
				StringBuilder sb = new StringBuilder(
						"SELECT A.SUBNAME TREE_LABEL,A.* FROM JTMSSPROFIT_RELATION A WHERE A.SETNAME = ?");
				params.add(this.subSetName);

				return (ArrayList<TreeNodeEntity>) JtMssProfitRelation
						.repository().jdbcFindList(sb.toString(), params,
								JtMssProfitRelation.class);
			} else {
			...
			if (PlatformUtil.getCurrentUser() != null) {
				...
				if (rootId != null) {
					StringBuffer sb = new StringBuffer(
							"SELECT A.SUBNAME TREE_LABEL,A.* FROM JTMSSPROFIT_RELATION A WHERE A.SUBSETNAME = ?");
					List params = new ArrayList();
					...
					if (telecomRegionId != null) {
						...
						if (this.isAgent || this.isIbe) {
							...
							sb = new StringBuffer(
									"SELECT A.SUBNAME TREE_LABEL,A.* FROM JTMSSPROFIT_RELATION A WHERE 1 = 1");
						}
...
/*						    subSetName = PermissionUtil.getPermissionOrganizationId(
						...
						return (ArrayList<TreeNodeEntity>) JtMssProfitRelation
								.repository().jdbcFindList(sb.toString(),
										params, JtMssProfitRelation.class);
					}
				}
			}
			return null;
		...
		if (treeCalcVo != null && node != null) {
			...
			while (iterator.hasNext()) {
				...
				for (int i = 0; i < nodes.size(); ++i) {
					JtMssProfitRelation entity = new JtMssProfitRelation();
...
/*		Collections.sort(entities, new Comparator<TreeNodeEntity>() {
	...
	public boolean isSubOrganization(Long orgId, Long parentOrgId) {
		...
		List<JtMssProfitRelation> list = JtMssProfitRelation.repository()
